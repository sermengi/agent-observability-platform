...
- **`POST /v1/regressions` runs against a scripted mock target, not a real agent.** `_mock_target()` (`routes/regressions.py`) replays the same `healthy_success` fixture for every scenario (GS-08 aside), which doesn't actually satisfy most golden scenarios' tool contracts — a run today reports a broadly-failing suite regardless of what's under test. This is an intentional stub pending Phase 10's real `AgentTarget`, not a signal to act on; treat `/v1/regressions` results as orchestration-plumbing validation only until then.
