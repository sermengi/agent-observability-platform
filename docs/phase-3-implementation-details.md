...
- **Known limitation:** `spans` and `tool_calls` carry a real, persisted `sequence` column set at ingestion time, so their `sequence` values are ID-stable across re-fetches. `LLMCall` has no such column in either the DB model or the `telemetry.v1` wire contract, so `LLMCallResponse.sequence` is a rank computed at query time (`ORDER BY started_at, llm_call_id`, enumerated from 1) rather than a persisted, ingestion-time value. It is stable as long as `started_at`/`llm_call_id` ordering doesn't change between requests, but it is not the same guarantee spans/tool_calls provide. Accepted as-is for Phase 3; revisit only if a future consumer needs ID-stable `llm_calls` ordering equivalent to spans/tool_calls.
